...
# Load your locally stored NIRS package
# Load your locally stored NIRS package
# Clear old functions from global environment if they exist (to avoid conflicts)
suppressWarnings(rm(list = c("plot_rawdata", "plot_regressions", "ggook"), envir = .GlobalEnv))
...
remove_wavelength <- function(rawdata, wave) {
  ...
  # Get the names of columns to keep: "samp_num" + the kept measurement columns
  # Note: rawdata$data usually contains "samp_num" plus the columns in info$cols
  cols_to_keep <- c("samp_num", cols_df$col_name[keep])

  # subset the data columns safely by name
  # ensure we only select columns that actually exist (defensive)
  actual_cols <- intersect(cols_to_keep, names(rawdata$data))
  rawdata$data <- as.data.frame(rawdata$data)[, actual_cols, drop = FALSE]
...
ggook_gui <- function(data, x = NULL, y = NULL) {
...
plot_regressions_gui <- function(models) {
  ...
  ggplot(df, aes(x = SectionZeroedTime, group = WavLabel)) +
    ...
    geom_line(aes(y = value_norm, color = WavLabel), size = 1, alpha = 0.6) +
    geom_line(aes(y = predicted_norm, color = WavLabel),
...
plot_rawdata_gui <- function(rawdata) {
...
interact_rawdata_server <- function(input, output, session,
  ...
  output$ook_plot <- renderPlot(ggook_gui(plotdata()), res = 96)
...
generate_patient_plot <- function(pid, summary_df) {
  ...
  curve_plot <- ggplot(
    ...
    labs(
      title = paste("Recovery Curves for Patient", pid),
  ...
  tau_plot <- ggplot(
    ...
    labs(
      title = paste("Tau Progression for Patient", pid),
...
server <- function(input, output, session) {
  ...
  output$rawCyclePlot <- renderPlot(
    {
      req(filteredCycle())
      plot_rawdata_gui(filteredCycle())
    },
    height = 300
  )
  ...
  output$regCyclePlot <- renderPlot(
    {
      req(filteredCycle())
      plot_regressions_gui(analysis(filteredCycle()))
    },
    height = 300
  )
  ...
  output$summaryTable <- DT::renderDataTable(
    {
      ...
      df$Delete <- sprintf(
        '<button class="delete-btn" data-row="%d">✖</button>',
        seq_len(nrow(df))
      )
      df
    },
  ...
  observeEvent(input$delete_row, {
    idx <- as.numeric(input$delete_row)
    df <- globalSummaryData()
